...
import CheckInMedia from './CheckInMedia';
...
const ViewCheckIn = ({
  ...
  env, intl,
  checkIn, initialRatings, ratings, checkInMedia,
  upVotes, downVotes,
  comments
}) => {
  ...
  const editCheckIn = (
    <div className={s.edit}>
      {
        checkIn.privateUuid &&
        <Link to={`/check-in/${checkIn.privateUuid}/edit`}>Edit</Link>
      }
    </div>
  );
  ...
  return (
    <div className={s.container}>
      ...
      <div className={s.instanceMediaSection}>
        <CheckInMedia checkIn={checkIn} media={checkInMedia} />
      </div>
      <Comments comments={comments} checkIn={checkIn} />
    </div>
  );
}
...
export default injectIntl(
  connect(state => ({
    ...
    env: state.env,
    ratings: state.viewCheckIn.ratings,
    upVotes: state.viewCheckIn.upVotes,
    downVotes: state.viewCheckIn.downVotes,
  }), {
    saveRating,
    setProperty,
    vote
